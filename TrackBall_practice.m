...
deg(deg < 0) = 0;
th = deg2rad(deg);
xx = radius*cos(th)+cir_center(1)-randi(200,20,1);
yy = cir_center(2)-radius*sin(th)+randi(200,20,1); %%randi(2,20,1)
% xx = radius*cos(th)+cir_center(1); 
% yy = cir_center(2)-radius*sin(th) + (randn(20,1)+10)*randi(30,1,1);
...
for i=1:numel(xx)
    ...
    DrawFormattedText(theWindow, '+', 'center', 'center', white, [], [], [], 2)
    Screen('Flip',theWindow);
    WaitSecs(2);
end
